...
class User:
    ...
    def encrypt_message(self, recipient_n, recipient_e, recipient_c, recipient_s):
        w = self.message
        if sympy.jacobi_symbol(w**2 - recipient_c, recipient_n) == 1:
            b1 = 1
            alpha = CoolNum(w, 1, recipient_c).div_with_cn(CoolNum(w, -1, recipient_c), recipient_n)
        else:
            b1 = -1
            alpha = (CoolNum(w, 1, recipient_c) * CoolNum(recipient_s, 1, recipient_c)).div_with_cn(CoolNum(w, -1, recipient_c)).div_with_cn(CoolNum(recipient_s, -1, recipient_c))
        
        if b1 == 1:
            a = (w ** 2 + recipient_c) * sympy.mod_inverse(w**2 - recipient_c, recipient_n) % recipient_n
        else:
            a = (((w ** 2 + recipient_c) % recipient_n) * ((recipient_s ** 2 + recipient_c) % recipient_n) + 4 * recipient_c * recipient_s * w)
